...
interface FeedDao {
    ...
    @Query("SELECT * FROM feed_items WHERE isBookmarked = 1 ORDER BY publishedAt DESC")
    fun getBookmarks(): Flow<List<FeedItemEntity>>

    @Query("SELECT * FROM feed_items WHERE isBookmarked = 1 ORDER BY publishedAt DESC")
    suspend fun getBookmarksOnce(): List<FeedItemEntity>

    @Query("SELECT * FROM feed_items WHERE title LIKE :query OR summary LIKE :query OR authorName LIKE :query ORDER BY publishedAt DESC")

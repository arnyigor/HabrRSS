...
class InMemoryFeedDao : FeedDao {
    ...
    override suspend fun countAllCachedPaged(
        ...
        items.matching(feedId, hubFilter, tagFilter, query, hideRead).size
    ...
    override suspend fun getByIds(ids: List<String>): List<FeedItemEntity> {
        ...
        return items.filter { it.id in wanted }
    }

    override suspend fun getExistingIds(ids: List<String>): List<String> {
        if (ids.isEmpty()) return emptyList()
        val wanted = ids.toHashSet()
        return items.asSequence().map { it.id }.filter { it in wanted }.toList()
    }
    ...
    private fun List<FeedItemEntity>.byFeed(feedId: String): List<FeedItemEntity> =
        filter { it.feedId == feedId }.sortedWith(
            compareBy<FeedItemEntity> { it.sourceOrder == null }
                ...
                .thenByDescending { it.publishedAtEpoch ?: it.fetchedAt },
        )
